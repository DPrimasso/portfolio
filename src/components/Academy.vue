  ...
  <div class="academy-app">
    ...
    <div v-else-if="screen === 's0'" class="ac-screen s0">
      <div class="s0-wrap">
        ...
        <h1 class="s0-title">Impara a programmare.<br /><em>Per davvero.</em></h1>
        <p class="s0-lead">
          Corsi gratuiti, sessioni guidate e percorsi su misura. Scegli il formato che fa per te.
        </p>
        ...
        <div class="s0-offers">
          <div class="s0-offer free">
            ...
            <div class="s0-offer-desc">
              Variabili, funzioni, cicli, array e oggetti. 8 moduli con esercizi e quiz per imparare
              JavaScript partendo da zero.
            </div>
          </div>
          ...
          <div class="s0-offer free">
            <div class="s0-offer-badge">gratuito</div>
            <div class="s0-offer-icon">{ }</div>
            <div class="s0-offer-title">Programmazione Avanzata</div>
            <div class="s0-offer-desc">
              OOP, closures, async/await, Fetch API, moduli ES6 e pattern avanzati. Il passo
              successivo dopo il corso base.
            </div>
          </div>
          ...
          <div class="s0-offer core">
            ...
            <div class="s0-offer-desc">
              Il mio prodotto principale. Scegliamo un progetto reale e lo sviluppiamo insieme,
              passo passo, vedendo i concetti uno alla volta messi in pratica. Ogni sessione = un
              pezzo di prodotto che funziona.
            </div>
          </div>
        </div>

        <div class="s0-actions">
          <button class="s0-login-btn" @click="goTo('s1')">→ accedi ai corsi</button>
          <button class="s0-back-btn" @click="emit('close')">← torna al portfolio</button>
        </div>
      </div>
    </div>
    ...
    <template v-else>
        ...
        <div class="s4-wrap">
          ...
          <div class="s4-module-list">
            <div class="s4-section-h">
              ...
              <div class="rule" />
              <div class="s4-module-filters">
                <button
                  :class="['module-filter-btn', { active: moduleFilter === 'all' }]"
                  @click="moduleFilter = 'all'"
                >
                  tutti
                </button>
                <button
                  :class="['module-filter-btn', { active: moduleFilter === 'pending' }]"
                  @click="moduleFilter = 'pending'"
                >
                  da fare
                </button>
                <button
                  :class="['module-filter-btn', { active: moduleFilter === 'completed' }]"
                  @click="moduleFilter = 'completed'"
                >
                  completati
                </button>
              </div>
            ...
            <div
              v-for="{ mod, mi, progress: modProg } in filteredModuleProgressList"
              :key="mod.id"
              class="module"
            ...
            <div v-if="!filteredModuleProgressList.length" class="s4-empty-filter">
      ...
      <div v-else-if="screen === 's5' && currentModule" class="ac-screen s5">
        ...
        <div class="s5-module-nav">
          <div class="s5-module-meta">
            modulo {{ currentModuleNumber }} / {{ currentCourse?.modules.length || 0 }}
          </div>
          <button class="s5-jump-btn" :disabled="!prevModule" @click="goToPrevModule">
            ← precedente
          </button>
          <select
            class="s5-module-select"
            :value="currentModule.id"
            @change="jumpToModuleById($event.target.value)"
          >
            <option v-for="(mod, mi) in currentCourse?.modules || []" :key="mod.id" :value="mod.id">
              {{ String(mi + 1).padStart(2, '0') }} — {{ mod.title }}
            </option>
          </select>
          <button class="s5-jump-btn primary" :disabled="!canGoNextModule" @click="goToNextModule">
            successivo →
          </button>
        </div>
...
function courseIcon(id) {
  return COURSE_META[id]?.icon ?? '{ }'
}
function courseColor(id) {
  return COURSE_META[id]?.color ?? ''
}
...
const filteredModuleProgressList = computed(() => {
  if (moduleFilter.value === 'completed') {
    return moduleProgressList.value.filter(item => item.progress?.completed)
  }
  if (moduleFilter.value === 'pending') {
    return moduleProgressList.value.filter(item => !item.progress?.completed)
  }
  return moduleProgressList.value
})
...
const currentModuleNumber = computed(() =>
  currentModuleIdx.value >= 0 ? currentModuleIdx.value + 1 : 0
)
const canGoNextModule = computed(
  () => !!nextModule.value && (alreadyCompleted.value || quizSubmitted.value)
)

function openModule(mod) {
  if (currentModule.value?.id === mod.id && screen.value === 's5') return
...
function jumpToModuleById(moduleId) {
  if (!currentCourse.value) return
  const target = currentCourse.value.modules.find(mod => mod.id === moduleId)
  if (target) openModule(target)
}

function goToPrevModule() {
  if (prevModule.value) openModule(prevModule.value)
}

function goToNextModule() {
  if (canGoNextModule.value && nextModule.value) {
    openModule(nextModule.value)
  }
}
...
.s0-offer.paid .s0-offer-badge {
  background: rgba(250, 204, 21, 0.1);
  color: #fbbf24;
  border-color: rgba(250, 204, 21, 0.25);
}

.s0-offer.core .s0-offer-badge {
